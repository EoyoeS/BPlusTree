...
struct Node
{
    bool is_leaf;                 // 是否是叶节点
    Node *parent;                 // 父节点
    std::vector<int> keys;        // 键
    std::vector<Node *> children; // 子节点
    std::vector<int> values;      // 值
    Node(Node *x, bool y) : parent(x), is_leaf(y) {}
};

class BPlusTree
{
    int d = 0;  // 阶数（非叶节点存储的子节点的最大数量）
    int hd = 0; // 分裂时的分割点
    int hf = 0; // 删除后节点最小键数量
    Node *root;

public:
    BPlusTree(int d) : d(d), hd(d >> 1), hf((d - 1) >> 1), root(nullptr) {}
...
private:
    ...
    void _add(Node *parent, int k, Node *node, Node *new_node)
    {
